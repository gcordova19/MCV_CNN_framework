...
def parse_pixel(label):
    ...
    if label==2:
        return [192, 192, 128] # Column_Pole = White-Gray
    if label==3:
        return [64, 64, 128] # Fence = Purple-Blue
    if label==4:
        return [64, 64, 0] # Pedestrian = Brown
    if label==5:
        return [128, 64, 128] # Road = Soft Purple
    if label==6:
        return [192, 128, 128] # SignSymbol = Pink
    if label==7:
        return [0, 128, 192] # Bicyclist = Soft Blue
    if label==8:
        return [64, 0, 128] # Car = Strong Purple
    if label==9:
        return [128, 128, 0] # Tree = Yellow
    if label==10:
...
output_dir_images = '/home/grupo09/M5/MCV_CNN_framework/Camvid/train/images/'
output_dir_segmentations = '/home/grupo09/M5/MCV_CNN_framework/Camvid/train/segmentations/'
